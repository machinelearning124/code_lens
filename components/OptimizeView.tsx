...
import { OptimizeViewProps, InputType } from '../types';
import { LANGUAGES } from '../constants';

const OptimizeView: React.FC<OptimizeViewProps> = ({ data, updateData, onRun, onClear }) => {
    const { codeText, inputLanguage, optimizeResult, isLoading, inputType, imagePreviewUrl } = data;
        ...
        <main className="flex-1 flex flex-col bg-transparent p-0 overflow-hidden">
                ...
                <div className="flex flex-col gap-1">
                    <div className="flex items-center gap-3">
                        <span className="p-1.5 glass-pane rounded-lg shadow-sm">
                            <span className="material-icons-round text-lg text-primary">rocket_launch</span>
                        </span>
                        <h1 className="text-xl font-bold text-slate-900 dark:text-white text-glass-shadow">Optimize Code</h1>
                    </div>
            ...
            <div className="flex-1 flex flex-col min-h-0 px-8 overflow-hidden">
                ...
                <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-6 min-h-0 pb-6">
                    ...
                    <div className="flex flex-col h-full card-neon shadow-2xl">
                        <div className="flex-shrink-0 px-4 py-3 border-b border-white/10 flex justify-between items-center bg-white/30 dark:bg-black/20 backdrop-blur-md">
                            <span className="font-bold text-xs uppercase tracking-wider text-slate-600 dark:text-slate-300">Optimized Code</span>
                            <div className="flex space-x-1">
                                <button
                                    onClick={() => navigator.clipboard.writeText(optimizeResult?.optimizedCode || '')}
                                    ...
                                    title="Copy"
                                    disabled={!optimizeResult?.optimizedCode}
                                >
                                    <span className="material-icons-round text-lg">content_copy</span>
                                </button>
                                <button
                                    onClick={() => {
                                        const blob = new Blob([optimizeResult?.optimizedCode || ''], { type: 'text/plain' });
                                        const url = URL.createObjectURL(blob);
                                        const a = document.createElement('a');
                                        a.href = url;
                                        a.download = `optimized_code.${inputLanguage.toLowerCase()}`;
                                        a.click();
                        ...
                        <div className="flex-1 p-0 bg-white/40 dark:bg-slate-900/20 overflow-scroll code-scroll-area">
                            {isLoading ? (
                                <div className="w-full h-full flex items-center justify-center flex-col bg-white/50 dark:bg-black/20 backdrop-blur-sm">
                                    ...
                                    <p className="text-sm font-bold text-slate-900 dark:text-white animate-pulse">Optimizing...</p>
                                </div>
            ...
            <div className="flex-shrink-0 py-6 border-t border-white/10 flex justify-center z-20">
                <button
                    onClick={onRun}
                    disabled={isDisabled}
                    title={!isDisabled ? "Click to optimize code" : undefined}
                    className={`
